...
#### Récupérer un salon publique libre

Il est possible de récupérer l'ID d'un salon publique avec au moins une place restante en envoyant un message.

.Message lobby libre (client)
:===
Code:JWT
*0x18*:JWT généré par le serveur
_uint8_t_:string
:===


.Message réponse ID (serveur)
:===
Code:ID
*0x19*:ID d'un Lobby **ou 0**
_uint8_t_:_lobby_id_t_
:===

IMPORTANT: Si aucun Lobby publique n'est disponible, **renvoie l'ID 0**.
...
:===
Code:Case d'origine:Case Destination:Attaque gagnée:Troupes perdues (attaque):Troupes perdues (défense):Nb dés(atk):Dés(atk):Nb dés(def):Dés(def)
*0x51*:ID case:ID case:1=W 0=L:Troupes perdues:Troupes perdues:Nombre dés:Tableau:Nombre dés:Tableau 
_uint8_t_:_uint16_t_:_uint16_t_:_bool (1 octet)_:_uint16_t_:_uint16_t_:_uint16_t_:_uint8_t[]_:_uint16_t_:_uint8_t[]_
:===
