...
__attribute__((weak)) void Game::add_troops(Session const& player_asking, uint16_t dst_square, uint16_t nb_troops) {
    ...
    if (nb_troops > troop_gained())
        throw LogicException{0x41, 
        "Pas assez de troupes : Le joueur ne possede pas autant de troupes"};

    if (nb_troops > get_current_player().get_remaining_deploy_troops())
        throw LogicException{0x41, 
        "Pas assez de troupes : Le joueur ne lui reste pas autant de troupes"};

    get_current_player().set_remaining_deploy_troops( 
        get_current_player().get_remaining_deploy_troops() - nb_troops);
...
__attribute__((weak)) void Game::transfer(Session const& player_asking, uint16_t src_square, uint16_t dst_square, 
    ...
    if (m_waiting_transfer)
        ...
        if (!m_map.is_valid_square(src_square))
            ...
            "Mauvaise case d'origine : ID case d'origine innexistant"};
        ...
        if (!m_map.is_valid_square(dst_square))
            ...
            "Mauvaise case de destination : ID case destination innexistant"};
        ...
        if (!m_map.is_possible_transfer(get_current_player().get_tag(), src_square, dst_square))
            throw LogicException{0x61,
            "Mauvaise case de destination : Aucun chemin entre case d'origine et destination"};
...
__attribute__((weak)) void Game::skip(Session const& player_asking) {
    ...
    if (m_phase == Placement && get_current_player().get_remaining_deploy_troops() > 0)
        throw LogicException{0x71, 
        "Phase non passable : Le joueur n'a pas deployé toutes ses troupes"};
...
__attribute__((weak)) uint16_t Game::troop_gained() {
    return (get_current_player().get_nb_square() / 3) 
        + (get_current_player().get_area_points());
}
...
void Game::transfer_after_attack(Session const& player_asking, uint16_t src_square, 
    uint16_t dst_square, uint16_t nb_troops) {
    
    if (get_current_player().get_tag().compare(lobby().get_gamertag(player_asking)))
        throw LogicException{0x70, "Ce n’est pas votre tour"};
    
    if (get_square_owner_map(src_square).get_last_atk_square() != dst_square 
        || get_square_owner_map(src_square).get_square_from_last_atk() != src_square)
            throw std::string("Ce ne sont pas les territoires de l'attaque");
    
    if (!is_finished())
        throw std::string("La partie est finie");

    if (current_phase() != Attack)
        throw std::string("Mauvaise phase de jeu pour attaquer");
    
    if (get_square_owner_map(src_square) != get_current_player())
        throw std::string("Mauvaise case d'origine : La case n'appartient pas au joueur");
    
    if (get_square_owner_map(dst_square) != get_current_player())
        throw std::string("Mauvaise case de destination : La case n'apprtient pas au joueur");
    
    if (!m_map.is_neighbor_square(src_square, dst_square))
        throw std::string("La case destination n'est pas voisine");

    if (m_map.get_nb_troops_square(src_square) <= nb_troops)
        throw std::string("Pas assez de troupe");

    if (nb_troops == 0)
        throw std::string("Nombre de troupe nul");
    
    if (get_square_owner_map(src_square).is_already_transfered_after_atk())
        throw std::string("Transfert de troupe deja effectue");
...
__attribute__((weak)) uint16_t Game::time_left() const {  //TODO
    return 1;
} 